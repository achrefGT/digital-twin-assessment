...
class LCCSettings(BaseSettings):
    """LCC service configuration settings"""
    
    model_config = SettingsConfigDict(
        env_file=".env",
        case_sensitive=False,
        env_prefix="LCC_"  # Environment variables prefixed with LCC_
    )
    
    # Service identity
    service_name: str = "lcc-service"
    version: str = "0.1.0"
    ...
    kafka_consumer_group_id: str = "lcc-service"
    ...
    processing_timeout_seconds: int = 60  # LCC calculations can take longer
    ...
    # LCC Calculation Constants
    default_discount_rate: float = 0.08      # 8% default discount rate
    min_discount_rate: float = 0.01          # 1% minimum
    max_discount_rate: float = 0.30          # 30% maximum
    default_analysis_years: int = 10         # 10-year default analysis period
    min_analysis_years: int = 3              # 3-year minimum
    max_analysis_years: int = 20             # 20-year maximum
    
    # ROI normalization bounds by industry
    roi_bounds_manufacturing: tuple[float, float] = (0.0, 1.5)
    roi_bounds_energy: tuple[float, float] = (0.0, 2.0)
    roi_bounds_aerospace: tuple[float, float] = (0.0, 2.5)
    roi_bounds_automotive: tuple[float, float] = (0.0, 1.8)
    
    # Feature flags
    enable_detailed_metrics: bool = True
    enable_performance_monitoring: bool = True
    enable_request_logging: bool = True
    enable_industry_adjustments: bool = True
    ...
    @field_validator('default_discount_rate')
    @classmethod
    def validate_discount_rate(cls, v):
        if not (0.01 <= v <= 0.30):
            raise ValueError('Discount rate must be between 1% and 30%')
        return v
    
    @property
    def database_url(self) -> str:
        """Get database URL using shared utility"""
        return get_database_url("lcc")
    ...
    def get_topic_config(self) -> Dict[str, str]:
        ...
        return {
            "input": self.lcc_submission_topic,
            "output": self.lcc_scores_topic,
            "error": self.error_events_topic,
        }
    
    def get_roi_bounds_for_industry(self, industry: str) -> tuple[float, float]:
        """Get ROI normalization bounds for specific industry"""
        industry_bounds = {
            "manufacturing": self.roi_bounds_manufacturing,
            "energy": self.roi_bounds_energy,
            "aerospace": self.roi_bounds_aerospace,
            "automotive": self.roi_bounds_automotive,
        }
        return industry_bounds.get(industry.lower(), self.roi_bounds_manufacturing)


# Global settings instance
settings = LCCSettings()