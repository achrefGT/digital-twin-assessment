...
from .models import ResilienceInput, ResilienceResult
from .scoring import calculate_resilience_score
...
class ResilienceKafkaHandler:
    ...
    async def start(self):
        ...
        try:
            ...
            self.consumer = await create_kafka_consumer(
                topics=[settings.resilience_submission_topic],
    ...
    async def _process_message(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            # Check if this message contains resilience data
            if form_submission.domain != 'resilience':
                logger.debug(f"Ignoring message for domain: {form_submission.domain}")
                print(f"DEBUG: Wrong domain '{form_submission.domain}', expected 'resilience'")
                return
                
            assessment_id = form_submission.assessment_id
            logger.info(f"Processing resilience form submission for assessment {assessment_id}")
            print(f"DEBUG: Processing assessment_id: {assessment_id}")
            
            start_time = datetime.utcnow()
            
            # Parse resilience input
            logger.debug("Parsing resilience input")
            ...
            # Create result
            logger.debug("Creating ResilienceResult")
            print(f"DEBUG: Creating result for assessment {resilience_input.assessmentId}")
            result = ResilienceResult(
                assessmentId=resilience_input.assessmentId,
            ...
            print(f"DEBUG: Saving assessment {resilience_input.assessmentId} to database")
            
            assessment_data = {
                "assessment_id": resilience_input.assessmentId,
                "user_id": resilience_input.userId,
                "system_name": resilience_input.systemName,
                "overall_score": result.overallScore,
                "domain_scores": result.domainScores,
                "risk_metrics": result.riskMetrics,
                "raw_assessments": resilience_input.assessments,  # Database will handle JSON serialization
                "metadata": resilience_input.metadata,  # Database will handle JSON serialization
                "submitted_at": resilience_input.submittedAt
            }
            ...
            logger.info(f"Successfully processed resilience assessment {assessment_id} in {processing_time:.2f}ms")
            ...
            logger.error(f"Unexpected error processing resilience message for assessment {assessment_id}: {e}")
    ...
    def _parse_resilience_input(self, form_submission) -> ResilienceInput:
        """Parse form submission into resilience input model"""
        try:
            logger.debug(f"Parsing form submission: {form_submission}")
            print(f"DEBUG: _parse_resilience_input called with type: {type(form_submission)}")
            ...
            if 'assessments' not in form_data:
                logger.error("Missing 'assessments' field in resilience form data")
                print(f"DEBUG: ❌ Missing 'assessments' field. Available keys: {list(form_data.keys()) if isinstance(form_data, dict) else 'N/A'}")
                raise InvalidFormDataException("Missing 'assessments' field in resilience form data")
            ...
            print(f"DEBUG: assessments_data length: {len(assessments_data) if hasattr(assessments_data, '__len__') else 'No length'}")
            ...
            logger.debug(f"Created ResilienceInput: {resilience_input}")
            print(f"DEBUG: ✅ Successfully created ResilienceInput for assessment {resilience_input.assessmentId}")
            return resilience_input
            
        except Exception as e:
            logger.error(f"Failed to parse resilience form data: {e}")
            print(f"DEBUG: ❌ _parse_resilience_input failed: {type(e).__name__}: {e}")
            if hasattr(form_submission, '__dict__'):
                print(f"DEBUG: form_submission details: {form_submission.__dict__}")
            import traceback
            traceback.print_exc()
            raise InvalidFormDataException(f"Failed to parse resilience form data: {e}")
    
    async def _publish_domain_scored_event(self, result: ResilienceResult):
        ...
        try:
            ...
            await self.producer.send(settings.resilience_scores_topic, event.dict())
            logger.info(f"Published domain scored event for assessment {result.assessmentId}")
            print(f"DEBUG: ✅ Successfully published domain scored event to {settings.resilience_scores_topic}")
