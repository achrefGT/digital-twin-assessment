...
class SimplifiedResilienceScoring:
    """Simplified scoring system with default configurations"""
    ...
    def _calculate_scenario_risk(self, assessment: Any) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            if likelihood_num is None or impact_num is None:
                raise ScoringException(f"Invalid likelihood or impact values")
    ...
    def _calculate_domain_score(self, domain_assessment: DomainAssessment, domain_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Apply confidence weighting if available, but cap at 100
            confidence_scores = [
                details.get('confidence', 3) for details in scenario_details.values()
            ]
            avg_confidence = np.mean(confidence_scores) if confidence_scores else 3
            confidence_factor = avg_confidence / 3.0  # Normalize around 3 (medium confidence)
            
            # Apply confidence adjustment but cap the final score at 100
            confidence_adjusted_score = min(100.0, domain_resilience * confidence_factor)
            
            return {
                'mean_risk_score': round(mean_risk, 2),
                'raw_resilience_score': round(domain_resilience, 1),
        ...
        try:
            ...
            for domain_name, domain_assessment in assessments.items():
                try:
                    ...
                    base_score = domain_result['confidence_adjusted_score']
    ...
    def _generate_recommendations(self, detailed_metrics: Dict[str, Any]) -> List[str]:
        ...
        domain_scores = {
            domain: metrics['confidence_adjusted_score'] 
