...
class ELCASettings(BaseSettings):
    """ELCA service configuration settings"""
    
    model_config = SettingsConfigDict(
        env_file=".env",
        case_sensitive=False,
        env_prefix="ELCA_"  # Environment variables prefixed with ELCA_
    )
    
    # Service identity
    service_name: str = "elca-service"
    version: str = "0.1.0"
    ...
    kafka_consumer_group_id: str = "elca-service"
    ...
    processing_timeout_seconds: int = 60  # Longer timeout for LCA calculations
    retry_attempts: int = 3
    retry_delay_seconds: int = 5
    
    # ELCA scoring configuration
    elca_scale_min: int = 0
    elca_scale_max: int = 100
    default_assessment_years: int = 5
    
    # Brightway2/LCA configuration
    brightway_project_name: str = "elca_digital_twins"
    lca_database_reset: bool = False
    enable_impact_method_setup: bool = True
    
    # Health check settings
    health_check_timeout_seconds: int = 5
    kafka_health_check_enabled: bool = True
    database_health_check_enabled: bool = True
    brightway_health_check_enabled: bool = True
    
    # Feature flags
    enable_detailed_metrics: bool = True
    enable_performance_monitoring: bool = True
    enable_request_logging: bool = True
    enable_regional_adjustments: bool = True
    enable_industry_scaling: bool = True
    ...
    def get_topic_config(self) -> Dict[str, str]:
        ...
        return {
            "input": self.elca_submission_topic,
            "output": self.elca_scores_topic,
            "error": self.error_events_topic,
        }


# Global settings instance
settings = ELCASettings()